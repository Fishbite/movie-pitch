...
// function to get text output from `text-davinci-003`
async function getTxtDavi003Result(prompt, tokens = 25, temperature = 0.75) {
  // create completion end-point
  const response = await openai.createCompletion({
    model: "text-davinci-003", // hard-code the model to use
    prompt: prompt, // instruction to openai
    max_tokens: tokens, // No. of tokens to use
    temperature: temperature, // 1 = whacky ai 0.1 = tamed ai
  });

  // const output = response.data.choices[0].text.trim();
  // return the trimed text output
  return response.data.choices[0].text.trim();
}

async function fetchBotReply(outline) {
  const prompt = `Generate a short message to enthusiastically say an outline sounds terrifying. I'll need a while to think about it.
      ###
  ...
  const result = await getTxtDavi003Result(prompt, 60);
  console.log("fetchBotReply result:", result);
  movieBossText.innerText = result;
}

async function fetchSynopsis(outline) {
  const prompt = `Generate an engaging, professional and marketable movie synopsis based on an outline. Choose actors that would be morbidly suitable for each role.
  ###
  ...
  const synopsis = await getTxtDavi003Result(prompt, 700);
  document.getElementById("output-text").innerText = synopsis;
  fetchTitle(synopsis);
}

async function fetchTitle(synopsis) {
  const prompt = `create a horrifyingly gruesome short title based on this synopsis: ${synopsis}`;

  const title = await getTxtDavi003Result(prompt, 700);
...
async function fetchStars(synopsis) {
  ...
  const stars = await getTxtDavi003Result(prompt, 30);
...
async function fetchImagePrompt(title, synopsis) {
  const prompt = `Use a title and synopsis to generate a short description of a poster image for the movie. Don't include any character or actor names.
  ###
  ...
  const imagePrompt = await getTxtDavi003Result(prompt, 100);
